...
Последовательность команд:
	...
	3.Команды matlab:
ВАЖНО! После отправки любой команды доставать SD карту из переходника НЕЛЬЗЯ. Если это необходимо сделать, то после установки карты обратно в переходник необходимо перезапустить контроллер.
1) start_command - передача даты и времени, запуск датчика
2) stop_command - остановка датчика
3) send_command - отправка данных в matlab
4) erase_command - стирание последнего созданного файла

send_command может быть отправлена несколько раз в случае, если необходимо повторно считать данные с SD карты
